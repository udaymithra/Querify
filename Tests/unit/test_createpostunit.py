...
class CreatePostTests(unittest.TestCase):
    ...
    def test_create_post(self):
        with self.client:
            ...
            form_data = {
                'title': 'New Test Post',
                'editor': 'Content for the new test post'
            }
            self.client.post('/post', data=form_data, follow_redirects=True)

            # Verify that the new post has been created
            new_post = Post.query.filter_by(title='New Test Post').first()
